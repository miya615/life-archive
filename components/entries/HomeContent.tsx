...
const TIME_CONFIG: Record<Period, {
  ...
  noon: {
    greeting: "こんにちは",
    subGreeting: "今日も丁寧に過ごしていますか",
    message: "今の気持ちや出来事を、数年後の自分が読み返せるように残しましょう。",
    heroGlow: "rgba(251,191,36,0.12)",
    btnText: "今を記録する",
  },
  evening: {
    greeting: "おつかれさまです",
    subGreeting: "今日一日を振り返る時間です",
    message: "何気ない1日も、未来の自分には大切な記録です。今日を静かに残しましょう。",
    heroGlow: "rgba(244,114,182,0.10)",
    btnText: "今日を残す",
  },
  night: {
    greeting: "おかえり",
    subGreeting: "今日もよく頑張りました",
    message: "一日の終わりに、今日の記憶を静かに残しましょう。眠る前のほんの数分で。",
    heroGlow: "rgba(139,124,248,0.12)",
    btnText: "今日の記憶を残す",
  },
};
...
export function HomeContent({ entries, monthCount, todayCount, displayName }: Props) {
  ...
  return (
    <div className="space-y-8 lg:space-y-10">
          ...
          <div className="relative">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              <Link href="/entries/new">
                <motion.div
                  whileTap={{ scale: 0.96 }}
                  className="flex items-center gap-2.5 rounded-2xl text-white font-semibold cursor-pointer"
                  style={{
            ...
            {entries.slice(0, 6).map((entry, i) => (
              ...
              >
                <Link href={`/entries/${entry.id}`}>
                  <motion.div
                    whileTap={{ scale: 0.98 }}
                    className="glass overflow-hidden cursor-pointer flex flex-col h-full"
                    style={{ minHeight: 200 }}
                  >
                    ...
                    {entry.image_url ? (
                      ...
                      <div className="flex items-center justify-center relative overflow-hidden" style={{ height: 88 }}>
                        ...
                        <span className="relative text-4xl" style={{ opacity: 0.5 }}>
      ...
      {/* ══ REFLECTIONS (inline — replaces right panel) ══ */}
      <HomeReflections />
